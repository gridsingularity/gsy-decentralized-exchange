...
async fn test_listener_captures_event_from_chain() -> Result<()> {
	...
	let project = Project::builder().paths(paths).ephemeral().no_artifacts().build()?;
	...
	let contract =
		&contract_list.first().expect("No versioned contract found in artifact").contract;
	...
	let handler = MockHandler { received_hashes: received_store.clone() };
