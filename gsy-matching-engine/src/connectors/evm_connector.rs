...
fn partition_orders_by_market_slot(
    ...
    let mut order_books: BTreeMap<(primitives::types::H256, u64), (Vec<Order>, Vec<Order>)> =
        BTreeMap::new();

    for bid in bids {
        order_books
            .entry((bid.market_id, bid.time_slot))
            .or_default()
            .0
            .push(bid);
    }
    for offer in offers {
        order_books
            .entry((offer.market_id, offer.time_slot))
            .or_default()
            .1
            .push(offer);
    }
...
async fn fetch_open_orders_via_ewds_query(fallback_url: String) -> Result<PreparedOrders, Error> {
    ...
    let values: Vec<Value> = query_via_ewds(EwdsQueryRequest {
        operation: EwdsOperation::OrdersQuery,
        query_payload,
        request_topic_env: "EWDS_ORDERS_REQUEST_TOPIC",
        request_topic_default: "ordersQuery",
        response_topic_env: "EWDS_ORDERS_RESPONSE_TOPIC",
        response_topic_default: "ordersQueryResponse",
        response_client_id_env: "EWDS_MATCHING_ENGINE_CLIENT_ID",
        response_client_id_default: "gsymatchingengine",
...
mod tests {
    ...
    use primitives::types::{AccountId32, H256};
    ...
    fn order(
        ...
        Order {
            ...
            area_uuid: H256::from([id; 32]),
            market_id: H256::from([market; 32]),
            time_slot,
            creation_time: 1,
            energy: 1,
            energy_rate: 1,
