...
const COMMUNITY_NAME: &str = "Partial Community";
const BUYER_AREA: &str = "partialBuyer";
const SELLER_AREA_ONE: &str = "partialSellerOne";
const SELLER_AREA_TWO: &str = "partialSellerTwo";
...
fn forecast_for_area(market: &MarketTopologySchema, area_name: &str, energy_kwh: f64) -> ForecastSchema {
...
#[when("the Market Orchestrator opens the partial-trade Spot market")]
async fn wait_for_partial_market_to_open(world: &mut MyWorld) {
	let market_id = world.generate_market_id(COMMUNITY_NAME, MarketType::Spot);
	info!("Waiting for the Market Orchestrator to open the partial-trade Spot market {:?}...", market_id);
		...
		for ev in events.find::<gsy_node::orderbook_registry::events::MarketStatusUpdated>() {
			if let Ok(e) = ev {
				if e.0 == market_id && e.1 {
					info!("Partial-trade market {:?} opened on-chain.", market_id);
	...
	panic!("Timeout: the orchestrator did not open the partial-trade Spot market {:?}", market_id);
}
...
async fn submit_partial_orders(
	...
	let bid_forecast = forecast_for_area(&market, BUYER_AREA, bid_energy);
	let offer_one = forecast_for_area(&market, SELLER_AREA_ONE, -offer_one_energy);
	let offer_two = forecast_for_area(&market, SELLER_AREA_TWO, -offer_two_energy);
