...
async fn submit_preferred_partner_bid(
	...
	let register_order_tx = gsy_node::tx().orderbook_worker().insert_orders(vec![bid_order]);
...
async fn submit_preferred_partner_offer(
	...
	let register_order_tx = gsy_node::tx().orderbook_worker().insert_orders(vec![offer_order]);
...
async fn submit_cheaper_offer(world: &mut MyWorld, user_name: String, energy: f64, rate: f64) {
	...
	let offer_order = InputOrder::Offer(InputOffer {
		...
		attributes: Some(Attributes { trading_partner_id: None, energy_type: EnergyType::Clean }),
	});
...
async fn verify_partner_trade(
	...
	println!("Checking past 15 blocks from {} for OrderExecuted...", latest_block.number());
...
async fn verify_trade_price(world: &mut MyWorld, price: u64) {
	...
	let actual_rate = world.last_trade_rate.expect("No trade was recorded in the previous step");
	assert_eq!(actual_rate, expected_rate, "Trade price did not match the preferred rate");
...
async fn verify_residual_offer(world: &mut MyWorld, energy: u64) {
	...
	let mut block_sub = world.subxt_client.blocks().subscribe_finalized().await.unwrap();
