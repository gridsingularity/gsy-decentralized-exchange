...
async fn query_orders_for_market(world: &MyWorld, market_id: &str) -> Vec<EwdsOrderDto> {
...
async fn query_market_trades(world: &MyWorld) -> Vec<DbTradeSchema> {
    ...
    trades
        .into_iter()
        .map(|trade| DbTradeSchema::try_from(trade).expect("Invalid EWDS trade DTO"))
        .collect()
}
...
async fn verify_community_market_settlements(world: &mut MyWorld) {
    ...
    for attempt in 0..COMMUNITY_TRADE_POLL_ATTEMPTS {
        ...
        for trade in &scenario_trades {
            assert!(
                trade.market_id.eq_ignore_ascii_case(&trade.bid.market_id)
                    && trade.market_id.eq_ignore_ascii_case(&trade.offer.market_id),
                "Trade {} contains orders from different markets",
                trade.trade_uuid
            );
            assert_eq!(trade.time_slot, world.target_delivery_time);
            assert_eq!(trade.bid.time_slot, world.target_delivery_time);
            assert_eq!(trade.offer.time_slot, world.target_delivery_time);
        }
...
async fn verify_pay_as_clear_unmatched_orders(world: &mut MyWorld) {
    ...
    assert_eq!(
        unmatched_bid.order_status, "submitted",
    ...
    assert_eq!(
        unmatched_offer.order_status, "submitted",
