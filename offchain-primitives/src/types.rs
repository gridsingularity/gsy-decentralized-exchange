...
impl PayAsBid for MatchingData {
    ...
    fn pay_as_bid(&mut self) -> Vec<Self::Output> {
        ...
        for offer in &mut offers {
            for bid in &mut bids {
                if offer.offer_component.area_uuid == bid.bid_component.area_uuid || offer.offer_component.energy == 0 || bid.bid_component.energy == 0 {
                    continue;
                }
                ...
                let offer_energy = *available_order_energy.entry(offer_id).or_insert(offer.offer_component.energy);
                let bid_energy = *available_order_energy.entry(bid_id).or_insert(bid.bid_component.energy);
