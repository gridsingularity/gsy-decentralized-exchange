...
use crate::utils::parse_uuid_or_hex_bytes16;
            ...
            for message in messages {
                ...
                if let Ok(parsed_payload) = parsed {
                    if parsed_payload.request_id == request_id {
                        ...
                        eprintln!("EWDS polling took {:?}s (period: {:?})",
                                  started.elapsed().as_millis()/1000, poll_interval_ms);
...
pub async fn get_onchain_id_via_ewds(
    ...
    eprintln!("get_onchain_id_via_ewds return value: {:?}", result);
    Ok(result)
}
...
pub async fn get_onchain_id(
    ...
    let onchain_id_hex = get_onchain_id_via_ewds(offchain_id, id_type).await?;
    parse_uuid_or_hex_bytes16(&onchain_id_hex)
