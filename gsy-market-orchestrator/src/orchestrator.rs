...
use gsy_offchain_primitives::{
    ...
    db_api_schema::market::{IntelligentMarketSchema, MatchingAlgorithm},
    ...
    C: MarketChainClient,
{
    ...
    let community_ids = fetch_list_of_community_ids_via_ewds().await?;
    ...
    loop {
        ...
        // todo: maybe increase the sleep interval
        ...
        for community_id in community_ids.iter() {
            for rule in MARKET_RULES.iter() {
                let markets = fetch_list_of_markets_via_ewds(
                    rule.market_type as u8, current_delivery_secs, community_id.clone()).await?;
...
pub async fn open_close_market<C>(
    client: &C, now: u64, current_delivery_secs: u64, market: &IntelligentMarketSchema, rule: &MarketRule
) -> anyhow::Result<()>
where
    C: MarketChainClient + ?Sized,
{
