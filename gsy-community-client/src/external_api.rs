...
impl MeasurementInfluxDBConnection {
    ...
    async fn fetch_from_db(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> Vec<RawInfluxDBMeasurement> {
        let query_str = format!(r#"from(bucket: "fedecom")
          |> range(start: {}, stop: {})
          |> filter(fn: (r) => r["_measurement"] == "active_energy")
          |> filter(fn: (r) => r["sensor_id"] =~ /^FLEXO-.*/)
          |> filter(fn: (r) => not r["sensor_id"] =~ /^FLEXO-AIC-49.*/)
        "#, start_time.to_rfc3339_opts(SecondsFormat::Secs, true).to_string(),
            end_time.to_rfc3339_opts(SecondsFormat::Secs, true).to_string(),
        );

        println!("Query: {}", query_str);
        let request_body = InfluxRequestParams {
            query: query_str.clone(), org_id: CommunityClientConstants.FEDECOM_INFLUX_DB_ORG.clone() };
        ...
        let response = client
            ...
            .json(&request_body).send().await.unwrap();
    ...
    pub async fn read(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> HashMap<String, HashMap<DateTime<Utc>, InfluxMeasurementMeterData>> {
        let mut smart_meter_measurements: HashMap<String, HashMap<DateTime<Utc>, InfluxMeasurementMeterData>> = HashMap::new();
        ...
        for record in fetched_data.iter() {
            ...
            assert_eq!(sensor_id_tokens.clone().nth(0).unwrap().to_string(), "FLEXO".to_string());
            ...
            if !smart_meter_measurements.contains_key(&smart_meter_id) {
                smart_meter_measurements.insert(
                    smart_meter_id.clone(), HashMap::new());
                smart_meter_measurements.get_mut(&smart_meter_id).unwrap().insert(
                    record.time, InfluxMeasurementMeterData {
                        sensor_id: smart_meter_id.clone(),
                        time: record.time,
                        import_Wh: 0., export_Wh: 0., consumption_Wh: 0., export_pv_Wh: 0.
                    }
                );
            }

            // Get nested reference for smart meter id and timestamp
            let hashmap_reference = smart_meter_measurements.get_mut(&smart_meter_id).unwrap().get_mut(&record.time).unwrap();
