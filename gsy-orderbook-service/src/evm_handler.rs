...
impl GsyEventHandler for OrderbookEvmHandler {
	async fn handle_order_placed(&self, event: OrderPlacedFilter) -> Result<()> {
		info!("Processing EVM OrderPlaced: {:?}", hex::encode(event.order_hash));
		...
		let schema =
			DbOrderSchema { _id: order_id_str, status: OrderStatus::Open, order: order_enum };
	...
	async fn handle_order_cancelled(&self, event: OrderCancelledFilter) -> Result<()> {
		info!("Processing EVM OrderCancelled: {:?}", hex::encode(event.order_hash));
		let id_bson =
			mongodb::bson::to_bson(&format!("0x{}", hex::encode(event.order_hash))).unwrap();

		match self.db.orders().update_order_status_by_id(&id_bson, OrderStatus::Deleted).await {
