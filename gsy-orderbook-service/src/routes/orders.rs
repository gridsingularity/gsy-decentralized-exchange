...
async fn filter_orders_from_db(
        db: DbRef, orders_parameters: Query<OrdersParameters>) -> Result<Vec<DbOrderSchema>, Error> {
    if orders_parameters.market_id.is_none() &&
        ...
        db.get_ref().orders().filter_orders(
            orders_parameters.market_id.clone(),
            orders_parameters.start_time, orders_parameters.end_time).await
    }
}
